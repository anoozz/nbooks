!include "MUI2.nsh"
!define PRODUCT_NAME "NBooks Daily Time Recorder"
...
Section "${PRODUCT_NAME}" SecDummy
	...
	CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} ${PRODUCT_VERSION}.lnk" "$INSTDIR\bin\dtr.exe"
	CreateShortCut "$DESKTOP\${PRODUCT_NAME} ${PRODUCT_VERSION}.lnk" "$INSTDIR\bin\dtr.exe"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
	!insertmacro MUI_DESCRIPTION_TEXT ${SecDummy} "NBooks Daily Time Recorder"
!insertmacro MUI_FUNCTION_DESCRIPTION_END

Section "Uninstall"

	Delete "$INSTDIR\Uninstall.exe"
	Delete "$INSTDIR\bin\dtr.exe"
	Delete "$INSTDIR\bin\dtr.exe.config"
